        ...
        with self.client.get("/", catch_response=True) as response:
            if response.status_code == 200 and "Set-Cookie" in response.headers:
                ...
                if "shop-session-id" in self.client.cookies:
                    ...
                    logging.error(
                        "CRITICAL: Failed to manually set cookie even after extraction."
                    )
            else:
                response.failure(
                    "Could not establish a session. No Set-Cookie header received."
                )
...
class StagesShape(LoadTestShape):
    ...
    def __init__(self):
        ...
        for idx, s in enumerate(self.stages):
            logging.info(
                f"  stage[{idx}] -> duration={s['duration']}s, users={s['users']}, spawn_rate={s['spawn_rate']}"
            )
    ...
    def _generate_randomized_hour_cycle(self):
        ...
        while current_duration < CYCLE_DURATION_SECS:
            ...
            if (
                event_block is FINAL_MINOR_SPIKE_EVENT
                or event_block is FINAL_MAJOR_SPIKE_EVENT
            ):
    ...
    def tick(self):
        ...
        if current_cycle != getattr(self, "_last_cycle_index", -1):
            logging.info(
                f"Hour boundary detected: regenerating stage schedule for cycle {current_cycle}"
            )
            self.stages = self._generate_randomized_hour_cycle()
        ...
        for stage in self.stages:
            if time_in_cycle < stage["duration"]:
                ...
                if time_into_stage < self.RAMP_DURATION_SECS:
                    ramp_spawn_rate = (
                        target_users - previous_stage_users
                    ) / self.RAMP_DURATION_SECS
                    current_user_target = int(
                        previous_stage_users + (ramp_spawn_rate * time_into_stage)
                    )
