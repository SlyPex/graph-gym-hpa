...
def build_graph(metrics , traffic_metrics = traffic_metrics_df ,num_features = NUM_FEATURES_PER_SERVICE):
        ...
        try:
            ...
            if json_data.get('status') != "success":
                ...
                if retries < max_retries:
                    print(f"Retrying in {retry_sleep}s... (attempt {retries}/{max_retries})")
            ...
            for dest_name in destinations:
                ...
                if result and len(result) > 0:
                    try:
                        ...
                        traffic_value = float(result[0]['value'][1]) * 1000  # Convert to appropriate scale
                    except (IndexError, KeyError, ValueError) as e:
                        print(f"Error parsing result for {source_name}->{dest_name}: {e}")
                        traffic_value = np.random.uniform(10, 80)  # Fallback
                else:
                    print(f"No data found for {source_name}->{dest_name}, using fallback")
                    traffic_value = np.random.uniform(10, 80)  # Fallback
...
def get_traffic_between_services(method='simulated', prometheus_url=PROMETHEUS_URL, **kwargs):
    """
