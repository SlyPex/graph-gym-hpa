...
from gym_hpa.gnn.graphCreation import build_graph, get_traffic_between_services, graph_to_data
...
class OnlineBoutique(gym.Env):
    ...
    def __init__(self, k8s=False, waiting_period=0.3):
        ...
        logging.info(f"[Init] Env: {self.name} | K8s: {self.k8s} | Version {self.__version__}")
        ...
        self.observation_space = Box(low=-np.inf, high=np.inf, shape=(58,), dtype=np.float32)

        self.deploymentList = get_online_boutique_deployment_list(self.k8s, MIN_REPLICATION, MAX_REPLICATION)
    ...
    def step(self, action):
        ...
        if action > ACTION_DO_NOTHING:
            ...
            action_description = f"Service: {DEPLOYMENTS[deployment_id]}, Scale: {scaling_value:+}"

        self.take_action(deployment_id, scaling_value)

        # Wait for changes to apply in a real cluster
        if self.k8s and action != ACTION_DO_NOTHING and not self.constraint_min_pod_replicas and not self.constraint_max_pod_replicas:
    ...
    def reset(self, *, seed=None, options=None):
        ...
        self.deploymentList = get_online_boutique_deployment_list(self.k8s, MIN_REPLICATION, MAX_REPLICATION)
        self.previous_system_distance = calculate_system_distance(self.deploymentList, 0)
    ...
    def take_action(self, deployment_id, scaling_value):
        ...
        if scaling_value == 0:  # Corresponds to ACTION_DO_NOTHING
            ...
            self.deploymentList[deployment_id].terminate_pod_replicas(abs(scaling_value), self)
    ...
    def get_reward(self):
        ...
        current_system_distance = calculate_system_distance(self.deploymentList, self.previous_system_distance)
        ...
        if self.current_step == 1:
            ...
            for i, name in enumerate(DEPLOYMENTS):
                ...
                if not match.empty:
                    ...
                    fallback_match = filtered_df[filtered_df[f"{name}_num_pods"] == pods[i]]
