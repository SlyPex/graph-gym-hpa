...
def fetch_prom(query, retry_delay=0.2):
    ...
    try:
        ...
        if data["status"] != "success":
            print(f"Prometheus query failed with status '{data['status']}': {data.get('error', 'No error message')}")
            return []
...
def calculate_system_distance(deployment_list, previous_system_distance, w_latency=2.0, w_cost=1.0):
    """Calculates a weighted distance metric for the system state."""
    latency_result = fetch_prom(QUERY_LATENCY)
    current_latency = float(latency_result[0]['value'][1]) if latency_result else LATENCY_SLO
