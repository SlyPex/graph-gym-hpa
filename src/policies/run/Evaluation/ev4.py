...
def map_pods_to_deployments():
    """
    Build mapping: pod_name -> deployment_name using ownerReferences or labels.
    """
    mapping = {}
        ...
        if pod.metadata.owner_references:
            for owner in pod.metadata.owner_references:
                if owner.kind == "ReplicaSet":
                    # get Deployment name from replicaset (strip hash suffix)
                    rs_name = owner.name
                    # Deployment name is everything before the last dash
                    dep_name = "-".join(rs_name.split("-")[:-1])
        if not dep_name and "app" in pod.metadata.labels:
            dep_name = pod.metadata.labels["app"]
...
if __name__ == "__main__":
    ...
    with (
        ...
        while time.time() - start_time < DURATION:
            ...
            pod_map = map_pods_to_deployments()

            for pod, cpu in cpu_usage.items():
                dep = pod_map.get(pod, "unknown")
